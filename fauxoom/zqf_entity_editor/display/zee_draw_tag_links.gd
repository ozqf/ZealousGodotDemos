...
func _draw_outgoing_links() -> void:
	...
	for i in range(0, numFields):
		...
		colourI += 1
		var subjectTags:PackedStringArray = field.value.split(",", false, 0)
...
func _draw_incoming_links() -> void:
	...
	var subjectTags:PackedStringArray = _proxy.get_tags_field_value("tagcsv")
	...
	for i in range(0, numProxies):
		...
		for field in fields:
			if field.name == "tagcsv":
				continue
			var queryTags:PackedStringArray = field.value.split(",", false, 0)
			if !_intersects(subjectTags, queryTags):
				continue
			add_vertex(origin)
			add_vertex(dest)

func _intersects(a:PackedStringArray, b:PackedStringArray) -> bool:
	for txtA in a:
		for txtB in b:
			if txtA == txtB:
				return true
...
func draw_tags(subject:ZEEEntityProxy, targetTags:PackedStringArray) -> void:
