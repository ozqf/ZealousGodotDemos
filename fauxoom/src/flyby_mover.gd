...
func _get_transform_by_step(step:int) -> Transform3D:
	return _trackNodes[step % _trackNodes.size()].global_transform

func _calc_current_step_multiplier() -> float:
	var a:Transform3D = _trackNodes[_index].global_transform
	var b:Transform3D = _trackNodes[_get_next_track_index(_index)].global_transform
	var dist:float = a.origin.distance_to(b.origin)
	var step = 1 / (dist / _speed)
	return step

func _calc_step_multiplier(a:Transform3D, b:Transform3D) -> float:
	var dist:float = a.origin.distance_to(b.origin)
	var step = 1 / (dist / _speed)
	return step

func _calc_transform(weight:float) -> Transform3D:
	var a:Transform3D = _get_transform_by_step(_stepCount)
	var b:Transform3D = _get_transform_by_step(_stepCount + 1)
	var c:Transform3D = _get_transform_by_step(_stepCount + 2)
	var ab:Transform3D = a.interpolate_with(b, weight)
	var bc:Transform3D = b.interpolate_with(c, weight)
	var t:Transform3D = ab.interpolate_with(bc, 0.5)
	return t
