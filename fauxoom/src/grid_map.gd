...
# Spawn cell
#########################################################
func _spawn_cell(x:int, y:int, z:int, scale:float, posOffset:Vector3, map:Dictionary) -> void:
...
func _iterate_build() -> void:
	...
	while _state.tileCount < _state.tileTotal:
		...
		_spawn_cell(int(cellPos.x), int(y), int(cellPos.y), scale, posOffset, _map)
		_state.tileCount += 1
		count += 1
